...
#  endif // _CCCL_CUDA_COMPILER(NVCC, <, 13, 3)
        // Copy a subset of the first 16 bytes. cp.async.bulk.cp_mask is sm_100-only; sm_90 uses scalar byte stores.
        NV_IF_ELSE_TARGET(
          NV_PROVIDES_SM_100,
          (if (::cuda::ptx::elect_sync(~0)) {
...
#  endif // _CCCL_CUDA_COMPILER(NVHPC)
        ...
        // Copy a subset of the last 16 bytes. cp.async.bulk.cp_mask is sm_100-only; sm_90 uses scalar byte stores.
        NV_IF_ELSE_TARGET(
          NV_PROVIDES_SM_100,
          (if (::cuda::ptx::elect_sync(~0)) {
            ::cuda::ptx::cp_async_bulk_cp_mask(
