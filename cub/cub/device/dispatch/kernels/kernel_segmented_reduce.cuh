  ...
  int blocks_per_segment)
{
  ...
  {
    if (d_partial_out != nullptr)
    {
      // Two-phase partial/chunked reduction mode: each block processes one chunk of a segment
      ...
      if (tid == 0)
      {
        reduce::finalize_and_store_aggregate(
          d_partial_out + bid, reduction_op, reduce::empty_problem_init_t<InitT>{init}, block_aggregate);
      }
    }
    else
    {
      // Normal mode: one segment per block
